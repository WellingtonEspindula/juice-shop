...
async function createChallenges () {
  ...
  await Promise.all(
    challenges.map(async ({ name, category, description, difficulty, hint, hintUrl, mitigationUrl, key_challenge, disabledEnv, tutorial, tags }: Challenge) => {
      ...
      try {
        datacache.challenges[key_challenge] = await ChallengeModel.create({
          key_challenge,
          name,
          category,
